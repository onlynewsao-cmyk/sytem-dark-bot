...
async function startDark(isRetry=false){
  ...
  const doPairing = async (attemptNum=1)=>{
    try{
      ...
      while(pairingNumber.startsWith('0')) pairingNumber = pairingNumber.slice(1);
      if(!pairingNumber || pairingNumber.length < 8){
        console.error('❌ PAIRING_NUMBER inválido:', pairingNumber);
        return false;
      }
      if(pairingNumber === '244949926074'){
        console.log(`
🚨🚨🚨 ALERTA DE NÚMERO PADRÃO DETECTADO 🚨🚨🚨
Você está tentando parear com o número padrão do template (+244949926074).
Se este NÃO for o seu número, vá no painel do Render > Environment Variables
e altere PAIRING_NUMBER e BOT_NUMBER para o seu número real!
`);
      }
      console.log(`\n╔════════════════════════════════════════╗`);
      ...
      console.log(`⏳ Solicitando código ao servidor do WhatsApp...`);

      // aguarda socket estar aberto e estável
      let waitWs = 0;
      while(!sock?.ws?.isOpen && waitWs < 20){
        await delay(1000);
        waitWs++;
      }
      await delay(3000);
...
`);
      ...
      // se não conectar em 58s, renova o código automaticamente antes de expirar
      setTimeout(async ()=>{
        if(!sock?.user && !state.creds.registered && attemptNum < cfg.system.pairing.attempts){
          console.log('⏰ Código anterior expirando (60s). Gerando novo código fresco...');
      ...
      // Se for erro 428 ou timeout temporário do WS, tenta mais rápido (5s em vez de 15s)
      const waitTime = (/428|precondition|closed|timeout/i.test(e.message)) ? 5000 : cfg.system.pairing.retryMs;
      if(attemptNum < cfg.system.pairing.attempts){
        console.log(`🔁 Nova tentativa em ${waitTime/1000}s...`);
