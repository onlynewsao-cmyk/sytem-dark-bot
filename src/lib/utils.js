...
async function downloadQuoted(sock, msg){
  try {
    const m = unwrapMessage(msg)?.extendedTextMessage?.contextInfo?.quotedMessage || null;
    if(!m) return null;
    const ctx = unwrapMessage(msg).extendedTextMessage.contextInfo;
    const q = { key: { remoteJid: msg.key.remoteJid, id: ctx.stanzaId, participant: ctx.participant }, message: m };
...
function extractButtonId(msg){
  ...
  try {
    const p = m.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson;
    if(p){
      const j = JSON.parse(p);
      return j.id || j.rowId || j.selectedId || j.selected_row_id || j.selected_id || '';
    }
  } catch(_){}
  return '';
}
...
function extractMentioned(msg){
  return unwrapMessage(msg)?.extendedTextMessage?.contextInfo?.mentionedJid || [];
}

function sleep(ms){ return new Promise(r=>setTimeout(r,ms)); }

module.exports = { onlyNumbers, sameNumber, sameJid, participantIds, getGroupAdmins, isBotAdmin, isAdmin, parseCmd, parseCmdMulti, msToTime, fmtBytes, getBuffer, downloadQuoted, extractText, extractButtonId, unwrapMessage, checkSpam, isOwnerFull, extractMentioned, sleep };
