...
async function handle(sock, msg, db){
  ...
  if(!body) body = msg.message?.conversation || '';
  ...
  if(['menu','help','start','sytem','dark','comandos'].includes(cmd)){
    const menuTxt = `Olá @${sender.split('@')[0]}! Prefixos: ${prefixes.filter(Boolean).join(' ')}

${S.bullet} *DOWNLOADS ULTIMATE*
${pfx}play • ${pfx}ytmp4 • ${pfx}tiktok
${pfx}instagram • ${pfx}pinterest • ${pfx}pin
${pfx}mediafire • ${pfx}apk

${S.bullet} *IA SYSTEM DARK*
${pfx}copilot • ${pfx}ia • ${pfx}nano
${pfx}addai • ${pfx}gemini

${S.bullet} *MANGÁ*
${pfx}manga • ${pfx}ler

${S.bullet} *GRUPO ULTIMATE*
${pfx}ban • ${pfx}tempban • ${pfx}promover
${pfx}marcar • ${pfx}hidetag
${pfx}antilink • ${pfx}antistatus
${pfx}antimencao • ${pfx}antispam
${pfx}welcome • ${pfx}despedida
${pfx}statusgp • ${pfx}add

${S.bullet} *VIP / HOSPEDAGEM*
${pfx}vip • ${pfx}addvip • ${pfx}host
${pfx}addhost • ${pfx}myvip

${S.bullet} *INTERAÇÕES*
${pfx}abraço • ${pfx}beijo • ${pfx}tapa
${pfx}soco • ${pfx}matar • ${pfx}cafune

${S.bullet} *DIVERSÃO*
${pfx}ship • ${pfx}gay • ${pfx}chance
${pfx}chance100 • ${pfx}rank

${S.bullet} *PROCURADO*
${pfx}procurado • ${pfx}addprocurado

${S.bullet} *INFO*
${pfx}ping • ${pfx}dbstatus • ${pfx}uptime
${pfx}sobrebot • ${pfx}prefixo

${S.footer}`;
    await sendButtons(sock, jid, wrap('SYTEM DARK ULTIMATE v5', menuTxt), [
      { id: `${pfx}menulista`, text: '📋 Menu Lista' },
      { id: `${pfx}pinterest`, text: '📌 Pinterest' },
      { type:'url', text:'👑 Dono', url:'https://wa.me/244945280380', id:'owner' },
      { type:'copy', id:'copy', text:'📋 Copiar Prefix', copy: pfx }
    ], S.footer, msg);
    return;
  }
  ...
  if(['menulista','menuprincipal','menucompleto'].includes(cmd)){
    const sections = [
      { title:'🌌 SYTEM DARK ULTIMATE', rows:[
        { title:'⬇️ Downloads', rowId:`${pfx}downloads`, description:'YT, TikTok, Pinterest...' },
        { title:'🤖 IA Dark', rowId:`${pfx}menuia`, description:'Copilot, Nano, Gemini' },
        { title:'📖 Mangá', rowId:`${pfx}manga one piece`, description:'Ler mangá PDF' },
        { title:'👥 Grupo', rowId:`${pfx}menugrupo`, description:'Admin ultimate' },
        { title:'💎 VIP / Host', rowId:`${pfx}vip`, description:'Planos e hospedagem' },
        { title:'🎮 Interações', rowId:`${pfx}menudiversao`, description:'Abraço, beijo...' },
        { title:'🚨 Procurado', rowId:`${pfx}procurado`, description:'Sistema policial' },
        { title:'⚙️ Sistema', rowId:`${pfx}dbstatus`, description:'Status do bot' },
      ]},
      { title:'📥 Downloads Extras', rows:[
        { title:'📌 Pinterest', rowId:`${pfx}pinterest gatos`, description:'Buscar pin' },
        { title:'📸 Instagram', rowId:`${pfx}instagram`, description:'ig download' },
        { title:'🎵 Spotify', rowId:`${pfx}spotify`, description:'Music dl' },
      ]}
    ];
    return sendList(sock, jid, '🌌 SYTEM DARK ULTIMATE', 'Escolha uma categoria:', sections, 'ABRIR DARK', msg);
  }
...
${pfx}dado / ${pfx}moeda`);
  ...
  // ========== IA ==========
  if(cmd==='copilot'){
    if(!text) return reply('COPILOT','Use: '+pfx+'copilot sua pergunta');
  ...
  if(cmd==='addai' || cmd==='metaai'){
    ...
    return reply(ok ? 'META AI':'ERRO', ok ? 'Meta AI adicionada!' : 'Falha ao adicionar');
  }
  if(cmd==='nano' || cmd==='deepedit'){
    if(await needHost()) return;
    const prompt = text || 'enhance quality, dark style';
    const buf = await U.downloadQuoted(sock, msg);
    if(!buf) return reply('NANO','Responda uma imagem com '+pfx+'nano <prompt>');
    await reply('NANO','Processando edição Dark...');
    try{
      const url = await AI.nanoEdit(buf, prompt);
      if(url && url.startsWith('http')){
        const img = await U.getBuffer(url);
        return sendImage(sock, jid, img, `✨ Nano Edit\n📝 ${prompt}\n\n${S.footer}`, msg);
      }
      return reply('NANO', String(url).slice(0,1000));
    }catch(e){ return reply('ERRO NANO', e.message); }
  }
  if(['ttkstalk','tiktokstalk','ttstalk'].includes(cmd)){
    const user = (args[0]||'').replace('@','');
    if(!user) return reply('TTKSTALK','Use: '+pfx+'ttkstalk usuario');
    try{
      const d = await Dl.tiktokStalk(user);
      const cap = `🌌 *TIKTOK STALK - SYTEM DARK*\n\n👤 ${d.nickname||''} (@${d.username||user})\n📝 ${d.bio||'-'}\n🔒 Privado: ${d.private?'Sim':'Não'}\n✔️ Verificado: ${d.verified?'Sim':'Não'}\n\n📊 Seguidores: ${d.followers}\n👥 Seguindo: ${d.following}\n❤️ Likes: ${d.likes}\n🎬 Vídeos: ${d.videos}\n\n🔗 https://tiktok.com/@${user}\n\n${S.footer}`;
      if(d.avatar) return sendImage(sock, jid, d.avatar, cap, msg);
      return reply('TTKSTALK', cap);
    }catch(e){ return reply('ERRO', e.message); }
  }
  ...
  // ========== DOWNLOADS ULTIMATE ==========
  if(['play','ytmp3','song'].includes(cmd)){
    const q = text;
    ...
    try{
      ...
      await sock.sendMessage(jid,{ text:`🎵 Baixando: ${vid.title}\n⏱️ ${vid.timestamp}\n\n${S.footer}` }, { quoted: msg });
  ...
  if(['ytmp4','ytplay4','video'].includes(cmd)){
    if(await needHost()) return;
    const url = args[0];
    if(!url) return reply('YTMP4','Use: '+pfx+'ytmp4 <url youtube>');
    try{
      await reply('DOWNLOAD','Baixando 1080p Dark...');
      const res = await Dl.ytmp4(url);
      await sock.sendMessage(jid, { video:{ url: res.url }, mimetype:'video/mp4', caption:`🎬 ${res.title}\n⚡ ${res.quality}\n\n${S.footer}` }, { quoted: msg });
    }catch(e){ return reply('ERRO', e.message); }
  }
